...
(function (exports,main_core,ui_vue3) {
	...
	function domDropHandler(ref) {
	  ...
	  return function (dropResult, onDrop) {
	    ...
	    if (addedIndex !== null) {
	      ...
	      wrapper.appendChild(removedWrapper && removedWrapper.firstElementChild ? removedWrapper.firstElementChild : element);
	...
	var getScrollingAxis = function getScrollingAxis(element) {
	  ...
	  if (xScroll) {
	    return ScrollAxis$1.x;
	  }
	  if (yScroll) {
	    return ScrollAxis$1.y;
	  }
	  return null;
	};
	...
	var isMobile$1 = function isMobile$1() {
	...
	var createAnimator = function createAnimator(element, axis) {
	  if (axis === void 0) {
	    axis = 'y';
	  }
	  var request = null;
	...
	function dragScroller(containers, maxScrollSpeed) {
	  if (maxScrollSpeed === void 0) {
	    maxScrollSpeed = maxSpeed;
	  }
	...
	function convertToCssString(css) {
	  return Object.keys(css).reduce(function (styleString, propName) {
	    ...
	    if (babelHelpers["typeof"](propValue) === 'object') {
	...
	function removeGrabListeners() {
	  ...
	  window.document.removeEventListener(touchstart, onMouseDown, {
	    passive: false
	  });
	}
	...
	function handleDropAnimation(callback) {
	  ...
	  if (draggableInfo.targetElement) {
	    ...
	    var container$1 = containers.filter(function (p) {
	    ...
	    if (container$1) {
	      ...
	      if ((behaviour === 'move' || behaviour === 'contain') && (isCanceling || !removeOnDropOut) && container$1.getDragResult()) {
	        ...
	        if (!isVisible(rectangles.visibleRect) && isVisible(rectangles.lastVisibleRect)) {
	          ...
	          var prevDraggableEnd = removedIndex > 0 ? layout.getBeginEnd(container$1.draggables[removedIndex - 1]).end : layout.getBeginEndOfContainer().begin;
	...
	function fireOnDragStartEnd(isStart) {
	  ...
	  containers.forEach(function (p) {
	    if (container.getOptions().fireRelatedEventsOnly && p !== container) {
	      return;
	    }
	...
	function initiateDrag(position, cursor) {
	  if (grabbedElement !== null) {
	    if (grabbedElement.classList.contains('dndrop-not-draggable')) {
	      return;
	    }
	    _isDragging = true;
	    var container = containers.filter(function (p) {
	...
	function findDraggebleAtPos(ref) {
	  ...
	  var find = function find(draggables, pos, startIndex, endIndex, withRespectToMiddlePoints) {
	    if (withRespectToMiddlePoints === void 0) {
	...
	function handleDrop(ref) {
	  ...
	  return function (draggableInfo, ref, forDispose) {
	    var addedIndex = ref.addedIndex;
	    var removedIndex = ref.removedIndex;
	    if (forDispose === void 0) {
	      forDispose = false;
	    }
	    draggablesReset();
	    ...
	    if (draggableInfo && !draggableInfo.cancelDrop) {
	      if (draggableInfo.targetElement || getOptions().removeOnDropOut || forDispose) {
	        var indexNotNull = function indexNotNull(index) {
	          return index !== null;
	        };
	        var actualAddIndex = indexNotNull(addedIndex) ? indexNotNull(removedIndex) && removedIndex < addedIndex ? addedIndex - 1 : addedIndex : null;
	        var payload = draggableInfo.payload;
	        ...
	        var shouldHandleDrop = !draggableInfo.container.getOptions().fireRelatedEventsOnly || indexNotNull(removedIndex) || indexNotNull(actualAddIndex);
	        if (shouldHandleDrop) {
	        ...
	        var container = draggableInfo.container;
	        return getOptions().dropNotAllowed({
	          payload: payload$1,
	          container: container
	        });
	      }
	...
	function drawDropPlaceholder(ref) {
	  ...
	  return function (ref) {
	    ...
	    if (options.dropPlaceholder) {
	      var ref$1 = typeof options.dropPlaceholder === 'boolean' ? {} : options.dropPlaceholder;
	...
	function getTagProps(ctx, tagClasses) {
	  ...
	  if (tag) {
	    if (typeof tag === 'string') {
	      ...
	      return result;
	    } else if (babelHelpers["typeof"](tag) === 'object') {
	      var result$1 = {
	        value: tag.value || 'div',
	        props: tag.props || {}
	      };
	...
	function validateTagProp(tag) {
	  if (tag) {
	    ...
	    if (babelHelpers["typeof"](tag) === 'object') {
	      if (typeof tag.value === 'string' || typeof tag.value === 'function' || babelHelpers["typeof"](tag.value) === 'object') {
	        return true;
	      }
	    }
	    return false;
	...
	var mapOptions = function mapOptions(context) {
	...
	var Container = {
	  ...
	  render: function render() {
	    ...
	    return ui_vue3.h(tagProps.value, Object.assign({}, {
	      ref: 'container'
	    }, tagProps.props), this.$slots["default"]());
	  }
	};
	var wrapChild = function wrapChild(createElement, ctx) {
	  var tagProps = getTagProps(ctx, ['dndrop-draggable-wrapper', ctx.dragNotAllowed ? 'dndrop-not-draggable' : '']);
	  return createElement(tagProps.value, Object.assign({}, tagProps.props), ctx.$slots["default"]());
	};
	var Draggable = {
	  name: 'Draggable',
	  props: {
	  ...
	  render: function render() {
	    return wrapChild(ui_vue3.h, this);
	  }
	};
	...
	var applyDrag = function applyDrag(arr, dragResult) {
	  ...
	  var result = babelHelpers.toConsumableArray(arr);
	...
	var columnNames = ['Backlog', 'Doing', 'Finished'];
	var cardColors = ['#34495E', '#84B0DC', '#49627A', '#41B883', '#7096BB', '#97CAFC', '#6CC1C0', '#41B883', '#41B883', '#49627A'];
	...
	var scene = {
	  ...
	  children: generateItems(3, function (i) {
	    return {
	      id: "column".concat(i),
	      type: 'container',
	      name: columnNames[i],
	      ...
	      children: generateItems(+(Math.random() * 10).toFixed() + 5, function (j) {
	        return {
	          type: 'draggable',
	...
	var Cards = {
	  ...
	  methods: {
	    onColumnDrop: function onColumnDrop(dropResult) {
	      var scene$$1 = Object.assign({}, this.scene);
	      scene$$1.children = applyDrag(scene$$1.children, dropResult);
	      this.scene = scene$$1;
	    },
	    onCardDrop: function onCardDrop(columnId, dropResult) {
	      if (dropResult.removedIndex !== null || dropResult.addedIndex !== null) {
	        var _scene = Object.assign({}, this.scene);
	        var column = _scene.children.filter(function (p) {
	          return p.id === columnId;
	        })[0];
	        var columnIndex = _scene.children.indexOf(column);
	    ...
	    getCardPayload: function getCardPayload(columnId) {
	      ...
	      return function (index) {
	        return _this.scene.children.filter(function (p) {
	          return p.id === columnId;
	        })[0].children[index];
	      };
	    },
...
}((this.BX.Up.Tasks = this.BX.Up.Tasks || {}),BX,BX.Vue3));
